...
#### practice for adding centroids to the @data
harvest2 <- harvest_beds_ogr@data %>% 
  mutate(lon = cent.coord$x,
         lat = cent.coord$y)

# create a DF of centroids
cents <-  gCentroid(harvest_beds_ogr, byid = TRUE)
cent.coord<- as.data.frame(cents@coords)
...
# centroid, gCentroid(as(data, "Spatial"), byid = TRUE)
...
#### adding ID numbers isn't working, so we are binding the actual centroid lat lon to each @data for each shapefile
...
## but it did not, so instead we added centroid lat lon
