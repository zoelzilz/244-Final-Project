...
SWH_SB_Cavannovh1 <- read_csv("SWH_SB_Cavannovh1.csv")
...
crop_nov <- st_crop(nov_swh, c(xmin=-120.68, xmax=-117.25, ymin=32.15, ymax=34.71)) #look up exact bounding box from arpa-e ## zoe says agian, is this fixed?

crop_socal_ras2 <- fasterize(crop_nov, r1, field = "nov_ssh") #used cropped vector and new cropped raster; this method appears identical
crop_socal_ras2

kelploss_nov <- (150.422 * crop_socal_ras2) / (3.54 + crop_socal_ras2) ### here we added in equation to convert to kelp loss instead of swh

################ equation information ##########################

#S <- SWH_SB_Cavannovh1$Max_Wave_Height
#v <- SWH_SB_Cavannovh1$Percent_Loss
#mm <- data.frame(S,v)
...
nov_map <- tm_shape(kelploss_nov) +
  tm_raster(title = "November Kelp Biomass Percent Loss")+
...
tmap_save(nov_map, "Kelpinator/www/November.png", height = 10)
