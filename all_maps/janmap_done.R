...
crop_jan <- st_crop(jan_swh, c(xmin=-120.68, xmax=-117.25, ymin=32.15, ymax=34.71)) #look up exact bounding box from arpa-e ## zoe says agian, is this fixed?

crop_socal_ras2 <- fasterize(crop_jan, r1, field = "jan_ssh") #used cropped vector and new cropped raster; this method appears identical
crop_socal_ras2

kelploss_jan <- (150.422 * crop_socal_ras2) / (3.54 + crop_socal_ras2) ### here we added in equation to convert to kelp loss instead of swh
...
jan_map <- tm_shape(kelploss_jan) +
  tm_raster(title = "January Kelp Biomass Percent Loss")+
...
tmap_save(jan_map, "Kelpinator/www/january.png", height = 10)
