...
crop_feb <- st_crop(feb_swh, c(xmin=-120.68, xmax=-117.25, ymin=32.15, ymax=34.71)) #look up exact bounding box from arpa-e ## zoe says agian, is this fixed?

crop_socal_ras2 <- fasterize(crop_feb, r1, field = "feb_ssh") #used cropped vector and new cropped raster; this method appears identical
crop_socal_ras2

kelploss_feb <- (150.422 * crop_socal_ras2) / (3.54 + crop_socal_ras2) ### here we added in equation to convert to kelp loss instead of swh
...
feb_map <- tm_shape(kelploss_feb) +
  tm_raster(title = "February Kelp Biomass Percent Loss")+
...
tmap_save(feb_map, "Kelpinator/www/february.png", height = 10)
