...
crop_may <- st_crop(may_swh, c(xmin=-120.68, xmax=-117.25, ymin=32.15, ymax=34.71)) #look up exact bounding box from arpa-e ## zoe says agian, is this fixed?

crop_socal_ras2 <- fasterize(crop_may, r1, field = "may_ssh") #used cropped vector and new cropped raster; this method appears identical
crop_socal_ras2

kelploss_may <- (150.422 * crop_socal_ras2) / (3.54 + crop_socal_ras2) ### here we added in equation to convert to kelp loss instead of swh
...
may_map <- tm_shape(kelploss_may) +
  tm_raster(title = "May Kelp Biomass Percent Loss")+
...
tmap_save(may_map, "Kelpinator/www/May.png", height = 10)
