...
SWH_SB_Cavanocth1 <- read_csv("SWH_SB_Cavanocth1.csv")
...
crop_oct <- st_crop(oct_swh, c(xmin=-120.68, xmax=-117.25, ymin=32.15, ymax=34.71)) #look up exact bounding box from arpa-e ## zoe says agian, is this fixed?

crop_socal_ras2 <- fasterize(crop_oct, r1, field = "oct_ssh") #used cropped vector and new cropped raster; this method appears identical
crop_socal_ras2

kelploss_oct <- (150.422 * crop_socal_ras2) / (3.54 + crop_socal_ras2) ### here we added in equation to convert to kelp loss instead of swh

################ equation information ##########################

#S <- SWH_SB_Cavanocth1$Max_Wave_Height
#v <- SWH_SB_Cavanocth1$Percent_Loss
#mm <- data.frame(S,v)
...
oct_map <- tm_shape(kelploss_oct) +
  tm_raster(title = "October Kelp Biomass Percent Loss")+
...
tmap_save(oct_map, "Kelpinator/www/October.png", height = 10)
