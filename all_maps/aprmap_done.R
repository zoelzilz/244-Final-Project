...
crop_apr <- st_crop(apr_swh, c(xmin=-120.68, xmax=-117.25, ymin=32.15, ymax=34.71)) #look up exact bounding box from arpa-e ## zoe says agian, is this fixed?

crop_socal_ras2 <- fasterize(crop_apr, r1, field = "apr_ssh") #used cropped vector and new cropped raster; this method appears identical
crop_socal_ras2

kelploss_apr <- (150.422 * crop_socal_ras2) / (3.54 + crop_socal_ras2) ### here we added in equation to convert to kelp loss instead of swh
...
apr_map <- tm_shape(kelploss_apr) +
  tm_raster(title = "April Kelp Biomass Percent Loss")+
...
tmap_save(apr_map, "Kelpinator/www/april.png", height = 10)
