...
crop_mar <- st_crop(mar_swh, c(xmin=-120.68, xmax=-117.25, ymin=32.15, ymax=34.71)) #look up exact bounding box from arpa-e ## zoe says agian, is this fixed?

crop_socal_ras2 <- fasterize(crop_mar, r1, field = "mar_ssh") #used cropped vector and new cropped raster; this method appears identical
crop_socal_ras2

kelploss_mar <- (150.422 * crop_socal_ras2) / (3.54 + crop_socal_ras2) ### here we added in equation to convert to kelp loss instead of swh
...
mar_map <- tm_shape(kelploss_mar) +
  tm_raster(title = "March Kelp Biomass Percent Loss")+
...
tmap_save(mar_map, "Kelpinator/www/march.png", height = 10)
