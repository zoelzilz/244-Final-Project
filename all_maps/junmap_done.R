...
crop_jun <- st_crop(jun_swh, c(xmin=-120.68, xmax=-117.25, ymin=32.15, ymax=34.71)) #look up exact bounding box from arpa-e ## zoe says agian, is this fixed?

crop_socal_ras2 <- fasterize(crop_jun, r1, field = "jun_ssh") #used cropped vector and new cropped raster; this method appears identical
crop_socal_ras2

kelploss_jun <- (150.422 * crop_socal_ras2) / (3.54 + crop_socal_ras2) ### here we added in equation to convert to kelp loss instead of swh
...
jun_map <- tm_shape(kelploss_jun) +
  tm_raster(title = "June Kelp Biomass Percent Loss")+
...
tmap_save(jun_map, "Kelpinator/www/june.png", height = 10)
